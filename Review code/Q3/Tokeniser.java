...
public class Tokeniser {
    ...
    private Token getTokenAt(int index) {
        ...
        if (Character.isDigit(_buffer.charAt(index))) {
            if (Integer.parseInt(_buffer.substring(index, index + 1)) % 2 == 0) {
                return new Token(Token.Type.EVEN_NUMBER, _buffer.substring(index, index + 1));
            } else {
                return new Token(Token.Type.ODD_NUMBER, _buffer.substring(index, index + 1));
            }
        }
        // END YOUR CODE
        return null;
