...
public class RBTree<T extends Comparable<T>> {
	...
	private void insertRecurse(Node<T> root, Node<T> insertNode) {
		boolean smaller = insertNode.key.lastName.compareTo(root.key.lastName) < 0 ||
				(insertNode.key.lastName.compareTo(root.key.lastName) == 0 && insertNode.key.firstName.compareTo(root.key.firstName) < 0);
		...
		if (smaller) {
			if (root.left.key == null) {
				...
				insertRecurse(root.left, insertNode);
			}
		} else {
			if (root.right.key == null) {
				...
				insertRecurse(root.right, insertNode);
			}
		}
	...
	private Node<T> searchRecursive(Node.PersonName<T> key, Node<T> rootNode) {
		if (key == null || rootNode == null || rootNode.key == null) {
			return null;
		} else if (key.firstName.compareTo(rootNode.key.firstName) == 0 && key.lastName.compareTo(rootNode.key.lastName) == 0) {
			return rootNode;
		} else if (key.lastName.compareTo(rootNode.key.lastName) < 0 ||
				(key.lastName.compareTo(rootNode.key.lastName) == 0 && key.firstName.compareTo(rootNode.key.firstName) < 0)) {
			return searchRecursive(key, rootNode.left);
		} else {
			return searchRecursive(key, rootNode.right);
		}
	}
