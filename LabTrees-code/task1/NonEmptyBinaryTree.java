...
public class NonEmptyBinaryTree <T extends Comparable<T>> extends BinaryTree<T> {
	...
	public BinaryTree<T> insert(T d) {
		...
		NonEmptyBinaryTree<T> tempNode = new NonEmptyBinaryTree<T>(data,left,right);
		if (d == data){
			return null;
		}else if (d.compareTo(data)<0){
			tempNode.left = left.insert(d);
		}else tempNode.right = right.insert(d);
		return tempNode;
		// ########## YOUR CODE ENDS HERE ##########
	}
	...
	public BinaryTree<T> delete(T d) {
		...
		if (data==null){
			...
			tempNode.left = left.delete(d);
			return tempNode;
		}else if (d.compareTo(data)>0){
			tempNode.right = right.delete(d);
			return tempNode;
		}else {
			if (tempNode.left.isEmpty()){
				return tempNode.right;
			}else if (tempNode.right.isEmpty()){
				return tempNode.left;
			}else {
				tempNode.data=tempNode.right.smallest();
				tempNode.right = tempNode.right.delete(tempNode.data);
			}
		}
		return tempNode;
		// ########## YOUR CODE ENDS HERE ##########
